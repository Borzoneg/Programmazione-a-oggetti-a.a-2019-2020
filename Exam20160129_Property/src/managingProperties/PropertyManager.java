...
public class PropertyManager {
	...
	if(! professionals.containsKey(profession))
	    throw new PropertyException(profession + " non registrato");
	...
	buildings.values().stream().forEach(b ->{
	    if(b.getCharges().size() > 0)
		result.put(b.getName(), b.getCharges());
	});
	return result;
    }
