...
public class Delivery {
    ...
    public String customerInfo(int customerId){
        return customers.get(customerId - 1).toString();
    }
	...
	customers.stream().forEach(c -> result.add(c.toString()));
    ...
    public void newMenuItem(String description, double price, String category, int prepTime){
        Food f = new Food(description, price, category, prepTime);
        foods.put(description, f);        
    }
	...
	List<String> result = new ArrayList<>();
        for(String s : foods.keySet()) {
        	...
        	result.add(foods.get(s).toString());
        }
        result.sort((s1, s2) -> s1.compareTo(s2));
        return result;
    }
	...
	Order o = new Order(customers.get(customerId - 1));
	orders.add(o);
	customers.get(customerId - 1).addOrder(o);
        return orders.size();
    }
	...
	for(Food f : foods.values()) {
            if(f.getDescription().toLowerCase().contains(search.toLowerCase()))
        	result.add(f);
        }
	if(result.size() != 1)
	    throw new DeliveryException();
	if(orderId > orders.size() || orderId < 0)
        ...
        orders.get(orderId-1).addFood(result.get(0), qty);
        return orders.get(orderId-1).getQuantity(result.get(0));
    }
	...
	if(orderId > orders.size() || orderId < 0)
	    throw new DeliveryException();	
	List<String> result = orders.get(orderId-1).foodsListString(); 
	result.sort((s1, s2) -> Integer.valueOf(s1.split(", ")[1]).compareTo(Integer.valueOf(s2.split(", ")[1])));
	return result;
    }
	...
	if(orderId > orders.size() || orderId < 0)
	    throw new DeliveryException();	
        return orders.get(orderId-1).getTotal();
    }
	...
	if(orderId < 1 || orderId > orders.size())
	    throw new DeliveryException();
	if(orders.get(orderId -1).getStatus() != OrderStatus.NEW)
	    throw new DeliveryException();
        return orders.get(orderId -1).confirm();
    }
	...
	if(orderId < 1 || orderId > orders.size())
	    throw new DeliveryException();
	if(orders.get(orderId -1).getStatus() != OrderStatus.CONFIRMED)
	    throw new DeliveryException();
	return orders.get(orderId -1).start();
    }
	...
	if(orderId < 1 || orderId > orders.size())
	    throw new DeliveryException();
	if(orders.get(orderId -1).getStatus() != OrderStatus.PREPARATION)
	    throw new DeliveryException();
	return orders.get(orderId -1).deliver();
    }
	...
	if(orderId < 1 || orderId > orders.size())
	    throw new DeliveryException();
	if(orders.get(orderId -1).getStatus() != OrderStatus.ON_DELIVERY)
	    throw new DeliveryException();
	orders.get(orderId -1).complete();
    }
	...
	return customers.get(customerId - 1).getTotal();
    }
	...
	SortedMap<Double,List<String>> result = new TreeMap<>();
	List<String> tmp ;
	for(Customer c : customers) {
	    if(! result.containsKey(c.getTotal()))
		tmp = new ArrayList<>();
	    else 
		tmp = result.get(c.getTotal());
	    tmp.add(c.toString());
	    result.put(c.getTotal(), tmp);
	}
        return result;
    }
