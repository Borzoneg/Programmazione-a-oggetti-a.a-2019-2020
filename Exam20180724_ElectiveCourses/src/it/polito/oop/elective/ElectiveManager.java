...
public class ElectiveManager {
    ...
    private Map<String, Course> coursesMap = new TreeMap<>();
	...
	SortedSet<String> result = new TreeSet<>();
        coursesMap.keySet().stream().forEach(name -> result.add(name));
        return result;
    }
	...
	if(students.containsKey(id))
	    students.get(id).setMedia(gradeAverage);
	else {
    ...
    public Collection<String> getStudents(double inf, double sup){
        return students.keySet().stream().filter(s -> students.get(s).getAvg() >= inf && students.get(s).getAvg() <= sup).collect(Collectors.toList());
    }
	...
	int i;
	if(courses.size() > 3 || courses.size()< 0)
	    throw new ElectiveException();
	if(! students.containsKey(id));
	for(i = 0; i<courses.size(); i++) {
	    if(! coursesMap.containsKey(courses.get(i)))	    
		throw new ElectiveException();
	    coursesMap.get(courses.get(i)).sceltoCome(i);
	}
    ...
    public Map<String,List<Long>> numberRequests(){
        ...
        coursesMap.values().stream().forEach(c -> result.put(c.getName(), c.getPreferenze()));
        return result;
    }
